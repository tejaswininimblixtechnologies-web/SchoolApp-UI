...
const TeacherDashboardLayout = ({ children, activePage = 'Dashboard', onNavigate }) => {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [teacherName, setTeacherName] = useState('Dr. Evelyn Reed');
  ...
  const menuItems = [
    ...
    { id: 'Marks/Grades', label: 'Marks/Grades', icon: GraduationCap },
      ...
      <div className="flex-1 lg:ml-64 flex flex-col h-screen">
        ...
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-gray-100 flex justify-between items-center px-8 sticky top-0 z-10">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold text-gray-900">{activePage === 'Dashboard' ? 'Teacher Dashboard' : activePage}</h1>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <button type="button" className="flex items-center gap-3 pl-6 border-l border-gray-100 hover:bg-gray-50 rounded-lg -ml-2 p-2 transition-colors z-20 relative">
              <div className="text-right hidden md:block">
                ...
                <p className="text-xs text-gray-500 font-medium">Mathematics Teacher</p>
              </div>
              <img
                src={profileImage || `https://ui-avatars.com/api/?name=${encodeURIComponent(teacherName)}&background=c7d2fe&color=3730a3`}
                alt="Profile"
